...
- [ ] Implement `POST /api/auth/forgot-password` - generate reset token, log to console (email TBD)
- [ ] Implement `POST /api/auth/reset-password` - validate reset token, update password
- [ ] Add input validation using Zod for all auth endpoints
...
## Aanvullende Taken (uit requirements-analyse)

### 5.1 Timezone-implementatie Frontend

- [ ] Implementeer timezone-conversie bij het tonen van logged_at tijden op basis van user.timezone
- [ ] Zorg dat date pickers in log-modals rekening houden met de gebruiker's tijdzone
- [ ] Converteer UTC-tijden uit de API naar lokale tijdzone bij weergave in Dashboard, History en Trends

### 5.2 Backfill Vorige Dagen

- [ ] Maak het mogelijk om vanuit de History-pagina een lege dag te selecteren en logs toe te voegen
- [ ] Valideer dat date pickers in log-modals correct werken bij het kiezen van een datum in het verleden
- [ ] Toon visuele indicatie in kalender/history welke dagen nog geen logs hebben

### 5.3 UX voor Cognitieve Toegankelijkheid

- [ ] Zorg voor grote, makkelijk te raken controls voor severity- en mood-scores (minimaal 48px touch targets)
- [ ] Minimaliseer het aantal stappen om een log in te voeren (max 2-3 taps)
- [ ] Gebruik duidelijke visuele feedback (kleuren, iconen) bij schalen in plaats van alleen cijfers
- [ ] Test de volledige log-flow op eenvoud en snelheid voor gebruikers met brain fog of vermoeidheid

### 5.4 Logout in Settings-scherm

- [ ] Voeg een Logout-knop toe aan het Settings-scherm zoals beschreven in de requirements

### 5.5 Insights Endpoint: Gemiddelden en Min/Max

- [ ] Breid `GET /api/insights/trends` uit met berekening van gemiddelde, minimum en maximum waarden per periode
- [ ] Retourneer gestructureerde statistieken (avg, min, max, count) per symptom/mood voor de geselecteerde datumrange
- [ ] Verbind de frontend Trends-pagina met deze statistieken en toon ze bij de grafieken
